    ...
    public static class Employee
    {
        ...
        public static EmpGenInfoModel GetEmpGenInfo(int gradeVal, int companyId, string empCode)
        {
            using (var con = new SqlConnection(Connection.ConnectionString()))
            {
                ...
                string sql = $"SELECT * FROM EmpGeneralInfo Egi WHERE (Egi.GradeValue=(CASE WHEN "+gradeVal+"<>-1 THEN "+gradeVal+ " ELSE Egi.GradeValue END)OR Egi.GradeValue IS NULL OR Egi.GradeValue=0)AND Active=1 AND EmpCode='" + empCode + "'AND CompanyID=" + companyId;
        ...
        {
            var con = new SqlConnection(Connection.ConnectionString());
            var query = $"SELECT EmpCode  FROM EmpGeneralInfo Egi WHERE Egi.GradeValue = (CASE WHEN "+ GradeValue +"<>-1 THEN "+ GradeValue+ " ELSE Egi.GradeValue END) AND Active=1 UNION SELECT EmpCode FROM EmpGeneralInfo WHERE GradeValue = '' AND Active=1";
        ...
        {
            var con = new SqlConnection(Connection.ConnectionString());
            var query = $"SELECT EmpCode  FROM EmpGeneralInfo Egi WHERE Egi.GradeValue = (CASE WHEN " + GradeValue + "<>-1 THEN " + GradeValue + " ELSE Egi.GradeValue END)  UNION SELECT EmpCode FROM EmpGeneralInfo WHERE GradeValue = ''";
            List<EmpSearchViewModel> employees = con.Query<EmpSearchViewModel>(query).ToList();
            return employees;
        }
        ...
        {
            var con = new SqlConnection(Connection.ConnectionString());
            var query = $"SELECT EmpCode  FROM EmpGeneralInfo Egi WHERE Egi.GradeValue = (CASE WHEN " + GradeValue + "<>-1 THEN " + GradeValue + " ELSE Egi.GradeValue END)";
            List<EmpSearchViewModel> employees = con.Query<EmpSearchViewModel>(query).ToList();
            return employees;
        }
    }
