    ...
    public partial class AttendenceApplicationController : ControllerBase
    {
        ...
        public IActionResult SaveOrUpdateAttendence(ManualAttendenceModel attnModel)
        {
            ...
            try
            {
                ...
                {
                    response.Status = AttendenceApplication.UpdateManualAttendence(attnModel);
                    if (response.Status)
                    {
                        response.Status = true;
                        response.Result = "Update Successfylly";
                    }
                    else
                    {
                        response.Status = false;
        ...
        [Authorize()]
        [HttpGet]
        [Route("api/v{version:apiVersion}/attendence/processattendencedata/process/date/{date}/comid/{comid}/branch/{branch}")]
        public IActionResult ProcessAttendenceData(string date, int comid, int branch)
        {
            Response response = new Response("/attendence/processattendencedata/process/date/{date}/comid/{comid}/branch/{branch}");

            try
            {
                response.Status = AttendenceApplication.ProcessAttendenceData(date, comid, branch);
                if (response.Status)
                {
                    response.Status = true;
