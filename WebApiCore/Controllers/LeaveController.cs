...
namespace HRMS.Controllers
{
    ...
    public class LeaveController : ControllerBase
    {
        ...
        public LeaveController(IMailer mailer)
        {
            _mailer = mailer;
        }
        ...
        public IActionResult GetLeaveStatus()
        {
            ...
            try
            {
                ...
                var leaveStatus = Leave.GetLeaveStatus(compId, grade, empCode, periodId, gender);
            ...
            try
            {
                ...
                if (isSuccess)
                {
                    using (var con = new SqlConnection(Connection.ConnectionString()))
                    {
                        ...
                        var email = con.QuerySingle<EmailModel>("SELECT Email FROM EmpContactInfo WHERE EmpCode ='" + leaveApply.ApplyTo + "'");
                        string body = string.Format("{0}, ID {1}", leaveApply.EmpName, leaveApply.EmpCode) + string.Format(" have applied a leave application for your kind approval." + Environment.NewLine + Environment.NewLine + "ENA GROUP");
                        string senderEmail = "hrpayroll@enagroup.net";
                        string senderName = "Ena Group";
                        string subject = "Leave Apply ";
                        if (email != null)
                        {
                            _mailer.SendEmailAsync(senderEmail, senderName, email.Email, subject, body);
            ...
            try
            {
                ...
                if (response.Status)
                {
                    using (var con = new SqlConnection(Connection.ConnectionString()))
                    {
                                ...
                                var email = con.QuerySingle<EmailModel>("SELECT Email FROM EmpContactInfo WHERE EmpCode ='" + ldvm.EmpCode + "'");
                        string body= string.Format("EMP ID: {0} ", ldvm.EmpCode) + Environment.NewLine + string.Format("Your leave application has approved by HRD from the Dated {0} to {1}." + Environment.NewLine + Environment.NewLine + "ENA GROUP", ldvm.StartDate.Date, ldvm.EndDate.Date);
                        string senderEmail = "hrpayroll@enagroup.net";
                        string senderName = "Ena Group";
                        string subject = "HR Approval Confirmation ";
                        if(email != null)
                        {
                            _mailer.SendEmailAsync(senderEmail, senderName, email.Email, subject, body);
